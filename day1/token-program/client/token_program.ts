...
class TokenInstruction {
  ...
  amount = 0;
  constructor(
    fields: { instruction: number; amount: number } | undefined = undefined
  ) {
    if (fields) {
...
class Token {
  ...
  supply = 0;
  constructor(
    fields: { authority: [32]; supply: number } | undefined = undefined
  ) {
    if (fields) {
...
class TokenAccount {
  ...
  amount = 0;
  constructor(
    fields: { owner: [32]; token: [32]; amount: number } | undefined = undefined
  ) {
    if (fields) {
 ...
 */
const NEW_TOKEN_SIZE = borsh.serialize(Token.schema, new Token()).length;
...
export async function createToken(): Promise<void> {
  ...
  if (masterTokenAccount === null) {
    ...
    let data = borsh.serialize(
      TokenInstructionNoAmount.schema,
      tokenInstruction
    );
    ...
    console.log(
      "Token successfully created at address ",
      tokenPubkey.toBase58()
    );
  } else {
    console.log(
      "Master Token already created at address ",
      tokenPubkey.toBase58()
    );
  }
  console.log(
...
export async function createNewKeyPair(seed: string): Promise<PublicKey> {
  ...
  if (tokenAcct === null) {
    ...
    console.log(
      "created account for Public Key ",
...
export async function createTokenAccounts(): Promise<void> {
  ...
  console.log(
    "Creating from and to accounts for token ",
    tokenPubkey.toBase58()
  );
...
export async function getAccountTokenInfo(account: PublicKey): Promise<void> {
  ...
  const accountInfo = borsh.deserializeUnchecked(
    TokenAccount.schema,
    TokenAccount,
    data
  );
  console.log(
    "account info for ",
    account.toBase58() + ": token address:",
    new PublicKey(accountInfo.token).toBase58(),
    ", balance:",
